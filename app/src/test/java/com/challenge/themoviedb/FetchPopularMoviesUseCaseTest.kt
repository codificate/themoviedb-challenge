...
class FetchPopularMoviesUseCaseTest {
    ...
    private val useCase = FetchPopularMoviesUseCase(api)

    @Test
    fun `WHEN use case is invoked the fetchPopularMovies api call SHOULD return a MoviesListResponse`() = runBlocking {
        // GIVEN
        whenever(api.fetchPopularMovies()).thenReturn(MOVIE_LIST_RESPONSE)
    ...
    @Test
    fun `WHEN use case is invoked the fetchPopularMovies api call SHOULD throw a HttpException`() = runBlocking {
        // GIVEN
        whenever(httpException.localizedMessage).thenReturn(HTTP_ERROR)
        whenever(api.fetchPopularMovies()).thenThrow(httpException)
    ...
    @Test
    fun `WHEN use case is invoked the fetchPopularMovies api call SHOULD throw a HTTP ERROR MESSAGE`() = runBlocking {
        // GIVEN
